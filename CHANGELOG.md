...
- **Naming Consistency (OrderItem)**: Renamed the legacy `Orderitem` identifier to `OrderItem` across the entire codebase (entity, repository, persistence, DTO, command, query, and HTTP handler layers) and renamed the source files (`orderitem_*.go` → `order_item_*.go`). The Go type now matches the DB table `order_items` and the `/orders/{order_id}/items` route. No API-contract change.
- **Migration 000003 Trigger Bug**: The `updated_at` trigger still referenced the old `orderitems` table after the rename to `order_items`; corrected the trigger name and `ON order_items` target so a fresh migration run succeeds.
- **Stale Telemetry Purge**: Cleared residual `/api/v1/order-items` series from the collector's in-memory spanmetrics and the Prometheus TSDB (volume wiped) so dashboards now surface only the live nested routes.
- **Database Schema Not Self-Healing**: After a Postgres data reset (e.g. volume/image cleanup forcing PG to re-initialize), the `orders`/`order_items` tables disappeared and every endpoint returned `relation "orders" does not exist`. Added GORM `AutoMigrate` on startup (`main.go`) for the `Order` and `OrderItem` entities so the schema is reconciled (create-only, never drops data) every boot. Re-applied the migration SQL to restore the live tables.
